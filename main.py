...
async def chat_completions(
    ...
    try:
        ...
        def generate():
            # =================== 最终正确逻辑 START ===================
            last_len = 0
            # 状态标志，用于识别是否已进入“答案”阶段 (处理完think块)
            ...
            for line in response.iter_lines():
                ...
                try:
                    ...
                    if msg_type == '0':
                        ...
                        if "<think>" in content and "</think>" in content and not in_answer_phase:
                            # 阶段一：这是<think>块
                            # 将其作为一个完整的增量发送
                            delta_to_send = content
                            # 切换到答案阶段，为下一个累积流做准备
                            in_answer_phase = True 
                            # 关键：重置长度计数器，因为答案流将从0开始累积
                            last_len = 0 
                        elif in_answer_phase:
                            # 阶段二：这是答案的累积流
                            # 只有当新内容比旧内容长时才计算增量
                            if len(content) > last_len:
                                delta_to_send = content[last_len:]
                                last_len = len(content)
                            # 如果长度不变或变短，说明是重复块或异常，忽略
                        else:
                            # 兼容没有<think>块的正常累积流
                            if len(content) > last_len:
                                delta_to_send = content[last_len:]
